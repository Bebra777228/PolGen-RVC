...
class TextEncoder(nn.Module):
    def __init__(self, input_dim, out_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, f0=True):
        ...
        self.emb_phone = nn.Linear(input_dim, hidden_channels).to(device)
        self.lrelu = nn.LeakyReLU(0.1, inplace=True).to(device)
        self.emb_pitch = nn.Embedding(256, hidden_channels).to(device) if f0 else None
        self.encoder = Encoder(hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout).to(device)
    ...
    def forward(self, phone, pitch, lengths):
        ...
        x_mask = torch.unsqueeze(sequence_mask(lengths, x.size(2)), 1).to(x.dtype).to(device)
...
class ResidualCouplingBlock(nn.Module):
    def __init__(self, channels, hidden_channels, kernel_size, dilation_rate, n_layers, n_flows=4, gin_channels=0):
        ...
        for i in range(n_flows):
            self.flows.append(ResidualCouplingLayer(channels, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=gin_channels, mean_only=True).to(device))
            self.flows.append(Flip().to(device))
...
class PosteriorEncoder(nn.Module):
    ...
    def forward(self, x, x_lengths, g=None):
        x_mask = torch.unsqueeze(sequence_mask(x_lengths, x.size(2)), 1).to(x.dtype).to(device)
        x = self.pre(x) * x_mask
...
class Generator(nn.Module):
    def __init__(self, initial_channel, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, gin_channels=0):
        ...
        for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernel_sizes)):
            self.ups.append(weight_norm(ConvTranspose1d(upsample_initial_channel // (2**i), upsample_initial_channel // (2 ** (i + 1)), k, u, padding=(k - u) // 2)).to(device))
...
class SourceModuleHnNSF(nn.Module):
    def __init__(self, sampling_rate, harmonic_num=0, sine_amp=0.1, add_noise_std=0.003, voiced_threshod=0, is_half=True):
        ...
        self.l_linear = nn.Linear(harmonic_num + 1, 1).to(device)
        self.l_tanh = nn.Tanh().to(device)
...
class GeneratorNSF(nn.Module):
    def __init__(self, initial_channel, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, gin_channels, sr, is_half=False):
        ...
        self.num_kernels = len(resblock_kernel_sizes)
        self.num_upsamples = len(upsample_rates)
        self.f0_upsamp = nn.Upsample(scale_factor=np.prod(upsample_rates)).to(device)
        self.m_source = SourceModuleHnNSF(sampling_rate=sr, harmonic_num=0, is_half=is_half).to(device)
        ...
        for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernel_sizes)):
            ...
            if i + 1 < len(upsample_rates):
                ...
                self.noise_convs.append(Conv1d(1, c_cur, kernel_size=stride_f0 * 2, stride=stride_f0, padding=stride_f0 // 2).to(device))
            else:
                self.noise_convs.append(Conv1d(1, c_cur, kernel_size=1).to(device))
...
class Synthesizer(nn.Module):
    def __init__(self, input_dim, spec_channels, segment_size, inter_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, spk_embed_dim, gin_channels, sr, **kwargs):
        ...
        self.enc_p = TextEncoder(input_dim, inter_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout).to(device)
        self.dec = GeneratorNSF(inter_channels, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, gin_channels=gin_channels, sr=sr, is_half=kwargs["is_half"]).to(device)
        self.enc_q = PosteriorEncoder(spec_channels, inter_channels, hidden_channels, 5, 1, 16, gin_channels=gin_channels).to(device)
        self.flow = ResidualCouplingBlock(inter_channels, hidden_channels, 5, 1, 3, gin_channels=gin_channels).to(device)
        self.emb_g = nn.Embedding(spk_embed_dim, gin_channels).to(device)
...
class Synthesizer_nono(nn.Module):
    def __init__(self, input_dim, spec_channels, segment_size, inter_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, spk_embed_dim, gin_channels, sr=None, **kwargs):
        super().__init__()
        self.segment_size = segment_size
        self.enc_p = TextEncoder(input_dim, inter_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, f0=False).to(device)
...
class MultiPeriodDiscriminator(nn.Module):
    def __init__(self, use_spectral_norm=False):
        ...
        discs = [DiscriminatorS(use_spectral_norm=use_spectral_norm).to(device)]
        discs = discs + [DiscriminatorP(i, use_spectral_norm=use_spectral_norm).to(device) for i in periods]
...
class DiscriminatorS(nn.Module):
    def __init__(self, use_spectral_norm=False):
        ...
        self.convs = nn.ModuleList([
            norm_f(Conv1d(1, 16, 15, 1, padding=7).to(device)),
            norm_f(Conv1d(16, 64, 41, 4, groups=4, padding=20).to(device)),
            norm_f(Conv1d(64, 256, 41, 4, groups=16, padding=20).to(device)),
            norm_f(Conv1d(256, 1024, 41, 4, groups=64, padding=20).to(device)),
            norm_f(Conv1d(1024, 1024, 41, 4, groups=256, padding=20).to(device)),
            norm_f(Conv1d(1024, 1024, 5, 1, padding=2).to(device))
        ])
        self.conv_post = norm_f(Conv1d(1024, 1, 3, 1, padding=1).to(device))
...
class DiscriminatorP(nn.Module):
    def __init__(self, period, kernel_size=5, stride=3, use_spectral_norm=False):
        ...
        norm_f = weight_norm if not use_spectral_norm else spectral_norm
        self.convs = nn.ModuleList([
            norm_f(Conv2d(1, 32, (kernel_size, 1), (stride, 1), padding=(get_padding(kernel_size, 1), 0)).to(device)),
            norm_f(Conv2d(32, 128, (kernel_size, 1), (stride, 1), padding=(get_padding(kernel_size, 1), 0)).to(device)),
            norm_f(Conv2d(128, 512, (kernel_size, 1), (stride, 1), padding=(get_padding(kernel_size, 1), 0)).to(device)),
            norm_f(Conv2d(512, 1024, (kernel_size, 1), (stride, 1), padding=(get_padding(kernel_size, 1), 0)).to(device)),
            norm_f(Conv2d(1024, 1024, (kernel_size, 1), 1, padding=(get_padding(kernel_size, 1), 0)).to(device))
        ])
        self.conv_post = norm_f(Conv2d(1024, 1, (3, 1), 1, padding=(1, 0)).to(device))
