...
class TextEncoder(nn.Module):
    def __init__(self, out_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, input_dim, f0=True):
        super().__init__()
...
class ResidualCouplingBlock(nn.Module):
    def __init__(self, channels, hidden_channels, kernel_size, dilation_rate, n_layers, n_flows=4, gin_channels=0):
        ...
        self.flows = nn.ModuleList([
            modules.ResidualCouplingLayer(channels, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels, mean_only=True),
            modules.Flip()
        ] * n_flows)
    ...
    def remove_weight_norm(self):
        for flow in self.flows:
            if isinstance(flow, modules.ResidualCouplingLayer):
...
class SynthesizerTrnMs(nn.Module):
    def __init__(self, spec_channels, segment_size, inter_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, spk_embed_dim, gin_channels, sr, text_encoder_cls, generator_cls, **kwargs):
        super().__init__()
        ...
        self.enc_p = text_encoder_cls(inter_channels, hidden_channels, filter_channels, n_heads, n_layers, kernel_size, p_dropout)
        self.dec = generator_cls(inter_channels, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, gin_channels=gin_channels, sr=sr, is_half=kwargs["is_half"])
...
class SynthesizerTrnMs256NSFsid(SynthesizerTrnMs):
    def __init__(self, **kwargs):
        super().__init__(text_encoder_cls=TextEncoder, generator_cls=GeneratorNSF, **kwargs)


class SynthesizerTrnMs768NSFsid(SynthesizerTrnMs):
    def __init__(self, **kwargs):
        super().__init__(text_encoder_cls=TextEncoder, generator_cls=GeneratorNSF, **kwargs)


class SynthesizerTrnMs256NSFsid_nono(SynthesizerTrnMs):
    def __init__(self, **kwargs):
        super().__init__(text_encoder_cls=TextEncoder, generator_cls=Generator, **kwargs)


class SynthesizerTrnMs768NSFsid_nono(SynthesizerTrnMs):
    def __init__(self, **kwargs):
        super().__init__(text_encoder_cls=TextEncoder, generator_cls=Generator, **kwargs)
