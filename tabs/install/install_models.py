...
def url_download():
    with gr.Tab("Загрузить по ссылке"):
        ...
        gr.HTML(
            "<h3>Поддерживаемые сайты: <a href='https://huggingface.co/' target='_blank'>HuggingFace</a>, <a href='https://pixeldrain.com/' target='_blank'>Pixeldrain</a>, <a href='https://drive.google.com/' target='_blank'>Google Drive</a>, <a href='https://mega.nz/' target='_blank'>Mega</a>, <a href='https://disk.yandex.ru/' target='_blank'>Яндекс Диск</a></h3>"
        )
...
def zip_upload():
    with gr.Tab("Загрузить ZIP архивом"):
        with gr.Row():
            ...
            with gr.Column(variant="panel"):
                gr.HTML(
                    "<h3>1. Найдите и скачайте файлы: .pth и необязательный файл .index</h3>"
                )
                gr.HTML(
                    "<h3>2. Закиньте файл(-ы) в ZIP-архив и поместите его в область загрузки</h3>"
                )
