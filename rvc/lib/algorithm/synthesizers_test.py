...
from rvc.lib.algorithm.f0 import F0Decoder, f0_to_coarse, normalize_f0
...
class Synthesizer(torch.nn.Module):
    ...
    def forward(
        ...
        if pitch is not None:
            lf0 = 2595.0 * torch.log10(1.0 + pitch.unsqueeze(1) / 700.0) / 500
            norm_lf0 = normalize_f0(lf0, x_mask, pitch)
    ...
    def infer(
        ...
        if pitch is not None:
            ...
            norm_lf0 = normalize_f0(lf0, x_mask, pitch)
            pred_lf0 = self.f0_decoder(m_p, norm_lf0, x_mask, spk_emb=g)
            pitch = (700 * (torch.pow(10, pred_lf0 * 500 / 2595) - 1)).squeeze(1)
