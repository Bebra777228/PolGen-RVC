...
def extract_zip(extraction_folder, zip_name):
    ...
    if not model_filepath:
        raise gr.Error(
            f"Не найден файл модели .pth в распакованном zip-файле. Пожалуйста, проверьте {extraction_folder}."
        )
...
def download_from_url(url, dir_name, progress=gr.Progress()):
    try:
        ...
        if "drive.google.com" in url:
            ...
            yandex_api_url = "https://cloud-api.yandex.net/v1/disk/public/resources/download?public_key={}".format(
                url
            )
            ...
            if response.status_code == 200:
                ...
                raise gr.Error(
                    f"Ошибка при получении ссылки на скачивание с Яндекс Диск: {response.status_code}"
                )
