        ...
        with gr.Tab("CoverGen"):
            ...
            with gr.Row():
                ...
                with gr.Column(scale=5):
                    with gr.Box():
                        with gr.Row():
                            convert_orig = gr.Checkbox(label='Без музыки')
                            convert_leed = gr.Checkbox(label='Основной вокал', value=True)
                            convert_backup = gr.Checkbox(label='Бэк-вокал')
                            convert_all = gr.Checkbox(label='Весь вокал')
                        ai_orig = gr.Audio(label='AI-кавер', visible=False)
                        ai_cover = gr.Audio(label='AI-кавер (основной вокал)', visible=False)
                        ai_cover_backing = gr.Audio(label='AI-кавер (основмной вокал и бэк-вокал)', visible=False)
                        ai_cover_all = gr.Audio(label='AI-кавер (весь вокал)', visible=False)
                        with gr.Accordion("Промежуточные аудиофайлы", open=False):
                            ai_vocals = gr.Audio(label='Преобразованный Вокал', visible=False)
                            ai_backing = gr.Audio(label='Преобразованный Бэк-Вокал', visible=False)
                            main_vocals_dereverb = gr.Audio(label='Вокал', visible=False)
                            backup_vocals = gr.Audio(label='Бэк-Вокал', visible=False)
                            all_vocals = gr.Audio(label='Весь вокал', visible=False)
                            instrumentals = gr.Audio(label='Инструментал', visible=False)
                ...
                with gr.Column(scale=1, min_width=100, min_height=100):
                    ...
                    clear_btn = gr.ClearButton(value='Сброс всех параметров', components=[use_hybrid_methods, convert_orig, convert_leed, convert_backup, convert_all], min_width=100, min_height=100)


            ref_btn.click(update_models_list, None, outputs=rvc_model)
            is_webui = gr.Number(value=1, visible=False)
            generate_btn.click(song_cover_pipeline,
                              inputs=[song_input, rvc_model, pitch, keep_files, convert_orig, convert_leed, convert_backup, convert_all, is_webui,
                                      main_gain, backup_gain, inst_gain, index_rate, filter_radius, rms_mix_rate, f0_method, crepe_hop_length,
                                      protect, reverb_rm_size, reverb_wet, reverb_dry, reverb_damping, reverb_width, low_shelf_gain, high_shelf_gain,
                                      limiter_threshold, compressor_ratio, compressor_threshold, delay_time, delay_feedback, noise_gate_threshold,
                                      noise_gate_ratio, noise_gate_attack, noise_gate_release, output_format, drive_db, chorus_rate_hz, chorus_depth,
                                      chorus_centre_delay_ms, chorus_feedback, chorus_mix, clipping_threshold],
                              outputs=[ai_orig, ai_cover, ai_cover_backing, ai_cover_all,
                                       ai_vocals, ai_backing, main_vocals_dereverb,
                                       backup_vocals, all_vocals, instrumentals])
            clear_btn.click(lambda: [0, 0, 3, 0.25, 0.33, 128, 0,
                            ...
                            outputs=[pitch, index_rate, filter_radius, rms_mix_rate, protect,
                                    ...
                                    ai_orig, ai_cover, ai_cover_backing, ai_cover_all,
                                    ai_vocals, ai_backing, main_vocals_dereverb,
                                    backup_vocals, all_vocals, instrumentals])
